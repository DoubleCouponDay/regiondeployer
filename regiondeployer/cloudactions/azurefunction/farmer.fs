...
[<Literal>]
let public basefarmername = "mmregionalfarmer"

[<Literal>]
let public maxfarmernamelength = 40

let public getappsettings() : Dictionary<string, string> =
...
let public getfunctionapps(logmessage:string -> unit): IEnumerable<IFunctionApp> = 
...
let rec public getrandomfarmername(logmessage:string -> unit) : string =
...
let private converthostnametoip (hostname:string) : string = //dns lookup using an external api
...
let public externalipalreadyexistsingroup (hostnametotest:string, resourcegroupname:string, logmessage:string -> unit) : bool =  
...
let rec public createfarmer (appname:string, regionname:string, existingresourcegroupname:string, existingstorageaccount:IStorageAccount, logmessage:string -> unit) : IFunctionApp =
...
let public deletesinglefarmer(farmerid:string, logmessage:string -> unit) : unit =
...
let public deletefarmers (farmername:string, logmessage:string -> unit) : int =    
